...
class FFNArch(DeepModel):
    typeof = "FFN"

    def __init__(self, options={}):
        super(FFNArch, self).__init__()
    ...
    def init_layers(self):
        self.bn1 = nn.BatchNorm1d(len(self.X.columns)).to(self.options["device"])

        self.lin_layers = nn.ModuleList(
            [
                nn.Linear(
                    len(self.X.columns) if i == 0 else self.options["fc_layers"][i - 1],
                    size,
